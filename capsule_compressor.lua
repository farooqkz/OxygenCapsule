...
local function init_capsule_filler(pos) 
    ...
    meta:set_string("infotext", "Oxygen Capsule filler")
...
minetest.register_node("oxygen_capsule:capsule_filler", { -- TODO: a better name
    description = "Capsule filler", 
    tiles = {"oxygen_capsule_filler.png"},
    groups = {cracky=2},
    ...
    on_timer = function(pos, elapsed)
        ...
        for i, v in ipairs(slots) do 
            ...
            if stackname == "oxygen_capsule:small_capsule_empty" or stackname ==
                "oxygen_capsule:small_capsule_half" then
                if stat == 10 then
                    if stackname == "oxygen_capsule:small_capsule_empty" then
                        stack:replace("oxygen_capsule:small_capsule_half")
                    else
                        stack:replace("oxygen_capsule:small_capsule_full")
                    end
                    stat = 0
                else
                    stat = stat + 1
...
minetest.register_abm({
    nodenames = {"oxygen_capsule:small_capsule_filler"},
    interval = 1.0,
    chance = 1,
    action = function(pos, node, active_obj_count, active_obj_count_wider)
        init_capsule_filler(pos)
    end
})
